...
/**
 * Event notification handler for macOS. Uses pthread condition variable and mutex
 * (via JNA's LibPThread) similarly to the Linux implementation.
 *
 * This mirrors EventNotificationHandlerLinux so the native SetEventNotification
 * callback can signal the condition variable and wake the waiting thread.
 */
public class EventNotificationHandlerMac extends AbstractEventNotificationHandler<EVENT_HANDLE> {

    private static final Logger LOGGER = LoggerFactory.getLogger(EventNotificationHandlerMac.class);
    ...
    protected void processMessages(byte[] bytes) {
        // Mirror other platforms: process incoming bytes if needed.
    }
